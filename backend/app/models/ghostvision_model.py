...
class GhostVisionModel(BaseModel):
    def __init__(self):
        super().__init__("ghostvision", "YOLO")
        self.model = None
        self.model_path = settings.ghostvision_path
        self.version = "1.0"
        self.class_names = ["Crab-Pot"]

    def load(self) -> Dict[str, Any]:
        if not self.model_path.exists():
            self.status = "ERROR"
            raise FileNotFoundError(f"GhostVision model not found: {self.model_path}")
